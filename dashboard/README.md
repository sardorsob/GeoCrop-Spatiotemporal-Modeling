...
- Task 1 phenology metrics (tone-coded RMSE / MAE / Coverage / CRPS tiles) and a Recharts NDVI seasonality chart with credible-interval band, posterior line, dashed empirical line, hover tooltips, and peak summary cards.
...
Last QA run on 2026-06-02 (after the `TASK-011` cleanup pass): `npm run test` passed 37/37 in 10 files, `npm run typecheck`, `npm run lint`, and `npm run build` passed.
