...
- `src/components/layout/TopBar.tsx` renders the sticky GeoCrop / U.S. Corn Belt identity, neutral paper action, and live source/task/load-issue badges.
- `src/components/layout/DashboardShell.tsx` owns URL-backed task/filter/pin state and renders one controlled Explore task tablist through one `TaskPanel` dispatcher. The measured `MapPanel` is injected only into Rotation; no Story composition, universal map, or global control wall remains.
- `src/components/layout/DashboardShell.test.tsx` verifies the empty-route Explore default, retired-mode URL compatibility, active task switching, visible legacy-layer warnings, neutral paper drawer, source continuity, and load errors.
...
- `src/lib/state/__tests__/url-state.test.ts` verifies Explore defaults, legacy `view` retirement, retired-layer normalization/warnings, invalid params, stable order, and unrelated-param preservation.
...
- `src/features/phenology/PhenologyPanel.tsx` composes Task 1 as one corn/soybean/winter-wheat comparator with crop emphasis and one compact shared season-window disclosure. The panel owns source/caveat and named fallback states.
...
- `src/features/rotation/RotationClassChart.tsx` allocates a largest-remainder 100-cell field from the dated Task 2 shares while printing exact two-decimal percentages, pixels, area, source date, and denominator. At wide widths the field sits beside a three-card stacked summary rail; narrow widths stack the same content.
...
- `src/features/extremes/ExtremesPanel.tsx` composes Task 3 as a matched flood/drought comparison. One of five direct `aria-pressed` crop buttons drives both event maps, and the complete event × state × crop table remains keyboard-reachable.
...
- TASK-004 adds URL-backed dashboard state helpers. `TASK-023` retires `view`; empty state opens Explore, old mode parameters are ignored and deleted on update, invalid params warn, and retired map-layer ids normalize visibly.
...
- TASK-010 wires the server/client boundary: filesystem artifact loading remains in the server page, while URL search params and interactions live in the client shell. `TASK-023` preserves that boundary while collapsing composition to Explore only.
- `DashboardShell` preserves unrelated URL params, makes Explore the empty-URL default, removes retired mode params on the next update, and keeps immediate local UI state synchronized with representative share URLs.
...
- Meaningful analytical state is URL-backed: active task, supported task-local filters, and selected evidence. Do not move it to localStorage.
...
- `npm audit --audit-level=high` reported zero vulnerabilities after `TASK-024`; the follow-up tasks added no dependency.
